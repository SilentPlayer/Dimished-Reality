...
// keep tracking on meshviewcontroller
@property STTracker *trackerRef;
...
- (void)saveObj
{
    ...
    NSString* zipFilename = @"Model.obj";
    NSString* objTemporaryFilePath = [documentDirectory stringByAppendingPathComponent:zipFilename];
    
    NSString* screenShotFilename = @"Preview.jpg";
    NSString* screenShotTemporaryFilePath = [documentDirectory stringByAppendingPathComponent:screenShotFilename];
    ...
    NSDictionary* fileWriteOptions = @{kSTMeshWriteOptionFileFormatKey: @(STMeshWriteOptionFileFormatObjFile) };
    
    // Temporary path for the zip file.
    
    NSError* error;
    BOOL success = [self.meshRef writeToFile:objTemporaryFilePath options:fileWriteOptions error:&error];
    if (!success)
    {
    ...
    NSURL* fileURL = [NSURL fileURLWithPath:objTemporaryFilePath];
